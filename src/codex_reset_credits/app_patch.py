...
def _reset_credit_js(
    ...
    return (
        ...
        'function __codexResetCreditSafeGet(){return import(new URL(__codexResetCreditModule,import.meta.url).href).then(module=>{let client=module[__codexResetCreditClientExport];if(!client?.safeGet)throw new Error("reset credit API client missing");return client.safeGet("/wham/rate-limit-reset-credits")})}'
        'function __codexResetCreditDirectFetch(){return fetch("https://chatgpt.com/backend-api/wham/rate-limit-reset-credits",{credentials:"include"}).then(response=>{if(!response.ok)throw new Error(`reset credit fetch failed ${response.status}`);return response.json()})}'
        'function __codexResetCreditFetch(){return __codexResetCreditSafeGet().catch(error=>{window.__codexResetCreditsLastError=String(error?.message||error);return __codexResetCreditDirectFetch()})}'
        ...
        'if(!rows.length)return;if(cachedResponse&&!force){__codexResetCreditRender(cachedResponse);return}__codexResetCreditFetch().then(response=>{delete window.__codexResetCreditsLastError,window[__codexResetCreditCacheKey]=response,__codexResetCreditRender(response)}).catch(error=>{window.__codexResetCreditsLastError=String(error?.message||error),cachedResponse?__codexResetCreditRender(cachedResponse):rows.forEach(__codexResetCreditHide)})}'
...
def _reset_row_needles() -> tuple[MenuNeedle, ...]:
    ...
    keyboard_shortcuts = MenuNeedle(
        text='h>0?(0,K.jsx)(F.Item,{RightIcon:ge,className:L(x&&`pl-[calc(var(--padding-row-x)+1.25rem)] pr-[var(--padding-row-x)]`),onClick:c,children:(0,K.jsx)(I,{id:`composer.mode.rateLimit.resetsAvailable`,defaultMessage:`{availableRateLimitResetCount, plural, one {# reset available} other {# resets available}}`,description:`Menu item for opening available rate limit resets`,values:{availableRateLimitResetCount:h}})}):null',
        credit_count="h",
        compact="x",
        jsx="K",
        item="F.Item",
    ...
    return (current, previous, updated, current_updated, remote_conversation, thread_shell, keyboard_shortcuts)
